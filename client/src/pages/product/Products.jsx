...
export const Products = ({
  ...
  const applyFiltersExcept = React.useCallback(
    (products, exceptKey) => {
      const f = filters || {};
      return products.filter((p) => {
        ...
        if (
          exceptKey !== "grindType" &&
          Array.isArray(f.grindType) &&
          f.grindType.length > 0
        ) {
          const productType = normalize(p.type);
    ...
    <>
      ...
      <div className="products-container">
        ...
        <main className="products-main">
          {loading && (
            <div style={{ marginBottom: 12, color: "#6b4a3e" }}>
              Cargando productos…
            </div>
          )}
          {error && (
          ...
          />
          <ProductsList products={visibleProducts} onAddToCart={onAddToCart} />
        </main>
      </div>
      <Footer />
    </>
