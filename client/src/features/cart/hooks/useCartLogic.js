    ...
    try {
      ...
      if (savedLocal.length > 0) {
        for (const it of savedLocal) {
          ...
          await api
            .post("/cart", { product_id: it.id, quantity: qty })
            .catch(() => {});
        }
      }
      ...
      const merged = Array.isArray(data2.items)
        ? data2.items.map(mapServerItem)
        : [];
  ...
  const removeItem = useCallback(
    (productId) => {
      ...
      if (getToken() && item?.cartItemId) {
        api.delete(`/cart/${item.cartItemId}`).catch(() => {});
      }
    },
    [items],
  );
    ...
    (productId, quantity) => {
      ...
      if (getToken() && item?.cartItemId) {
        if (qty <= 0) {
          ...
          api
            .put(`/cart/${item.cartItemId}`, { quantity: qty })
            .catch(() => {});
        }
      }
    },
